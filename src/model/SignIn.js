...
SignIn.prototype.checkPassword = function(data) {
  if (data.rows[0]) {
    ...
    bcrypt.compare(this.inputs.password, dbhash, (error, result) => {
      if (error) {
        consol.log(error, "SignIn.js");
      } else if (result == false) {
        this.res
          .status(200)
          .json({
            success: false,
            userNotify: "That email or password is invalid",
            userData: {}
          });
      } else if (result == true) {
        ...
        this.res
          .status(200)
          .json({ success: true, userNotify: "", userData: data.rows[0] });
      }
    });
  } else {
    ...
    this.res
      .status(200)
      .json({ userNotify: "That email or password is invalid", userData: "" });
  }
};
