...
const getBlogList = function(req, res, next) {
	Conn.query(
		"SELECT title, description, postid FROM posts WHERE published = 'true' ORDER BY postid DESC"
	)
		.catch((e) => {
			log(e, 'blog.js')
		})
		.then((resp) => {
			...
			data.forEach((elem) => {
				list +=
					'<div id="' +
					elem.postid +
					'" class="bloglist" ><span class="listTitle">' +
					elem.title +
					'</span><br/><span class="listDesc">' +
					elem.description +
					'</span></div>'
			})
			req.bloglist = data
...
const getCurrentPost = function(req, res, next) {
	if (req.query.postid) {
		...
		let query =
			"SELECT title, description, body, postid FROM posts WHERE published = 'true' ORDER BY postid DESC LIMIT 1"
		Conn.query(query)
			.catch((e) => {
				log(e, 'blog.js')
			})
...
const getPostByID = function(req, res) {
	var query = {
		text:
			"SELECT title, description, body, postid FROM posts WHERE postid=$1 AND published = 'true'",
